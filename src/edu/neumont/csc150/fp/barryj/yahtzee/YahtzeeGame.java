...
public class YahtzeeGame implements ControlListener {
		...
		if (rollCount < 3) {
			...
			for (int i = 0; i < 5; i++) {
				if (!dieHeld[i].isHeld()) {
					...
					dieHeld[i].setDieFace(new ImageIcon("/edu/neumont/csc150/fp/barryj/images/" + dieHeld[i].getDieSymbol() + ".jpg"));
	...
	public void lowScores(int i) {
		...
		for (int j = 0; j < 5; j++) {
			if (dieHeld[j].isHeld()) {
				dieHeld[j].setDieFace(new ImageIcon("/edu/neumont/csc150/fp/barryj/images/" + dieHeld[j].getDieSymbol() + ".jpg"));
